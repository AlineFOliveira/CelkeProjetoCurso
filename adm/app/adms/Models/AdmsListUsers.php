...
class AdmsListUsers
{
    ...
    public function listUsers(int $page = null): void
    {
        ...
        $pagination = new \App\adms\Models\helper\AdmsPagination(URL . 'list-users/index');
    ...
    public function searchUserName(): void
    {
        ...
        $pagination->pagination("SELECT COUNT(usr.id) AS num_result 
                            FROM adms_users usr
                            INNER JOIN adms_access_levels AS lev ON lev.id=usr.adms_access_level_id
                            WHERE usr.name LIKE :search_name AND lev.order_levels > :order_levels", "search_name={$this->searchNameValue}&order_levels=" . $_SESSION['order_levels']);
        $this->resultPg = $pagination->getResult();
        ...
        $listUsers->fullRead("SELECT usr.id, usr.name AS name_usr, usr.email, usr.adms_sits_user_id,
                ...
                ORDER BY usr.id DESC
                LIMIT :limit OFFSET :offset", "search_name={$this->searchNameValue}&order_levels=" . $_SESSION['order_levels'] . "&limit={$this->limitResult}&offset={$pagination->getOffset()}");
    ...
    public function searchUserEmail(): void
    {
        ...
        $pagination->pagination("SELECT COUNT(usr.id) AS num_result 
                            FROM adms_users usr
                            INNER JOIN adms_access_levels AS lev ON lev.id=usr.adms_access_level_id
                            WHERE usr.email LIKE :search_email AND lev.order_levels > :order_levels", "search_email={$this->searchEmailValue}&order_levels=" . $_SESSION['order_levels']);
        $this->resultPg = $pagination->getResult();
        ...
        $listUsers->fullRead("SELECT usr.id, usr.name AS name_usr, usr.email, usr.adms_sits_user_id,
                    ...
                    INNER JOIN adms_colors AS col ON col.id=sit.adms_color_id
                    INNER JOIN adms_access_levels AS lev ON lev.id=usr.adms_access_level_id 
                WHERE usr.email LIKE :search_email AND lev.order_levels > :order_levels
                ORDER BY usr.id DESC
                LIMIT :limit OFFSET :offset", "search_email={$this->searchEmailValue}&order_levels=" . $_SESSION['order_levels'] . "&limit={$this->limitResult}&offset={$pagination->getOffset()}");
