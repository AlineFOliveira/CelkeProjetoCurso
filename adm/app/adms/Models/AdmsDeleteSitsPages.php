...
class AdmsDeleteSitsPages
{
    ...
    public function deleteSit(int $id): void
    {
        ...
        if(($this->viewSit()) and ($this->checkStatusUsed())){
            ...
            $deleteUser->exeDelete("adms_sits_pgs", "WHERE id =:id", "id={$this->id}");
    ...
    private function checkStatusUsed():bool
    {
        ...
        if($viewUserAdd->getResult()){
            $_SESSION['msg'] = "<p class='alert-warning'>Erro: Situação não pode ser apagada, há páginas cadastrados com essa situação!</p>";
            return false;
        }else{
            return true;
        }
