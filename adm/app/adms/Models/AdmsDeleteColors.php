...
class AdmsDeleteColors
{
     ...
     * @return void
     */
    public function deleteCol(int $id): void
    {
        $this->id = (int) $id;
    ...
    private function checkColorUsed(): bool
    {
        ...
        $viewColorUsedPg = new \App\adms\Models\helper\AdmsRead();
        $viewColorUsedPg->fullRead("SELECT id FROM adms_sits_pgs WHERE adms_color_id =:adms_color_id LIMIT :limit", "adms_color_id={$this->id}&limit=1");
        if ($viewColorUsed->getResult() or $viewColorUsedPg->getResult()) {
            $_SESSION['msg'] = "<p class='alert-warning'>Erro: Cor não pode ser apagada, há situações cadastrada com essa cor!</p>";
            return false;
        } else {
            return true;
        }
