        ...
        <div class="top-list">
            ...
            <div class="top-list-right">
                ...
                if ($this->data['button']['list_conf_emails']) {
                    echo "<a href='" . URL . "list-conf-emails/index' class='btn-info'>Listar</a> ";
                }
                if (!empty($this->data['viewConfEmails'])) {
                    if ($this->data['button']['edit_conf_emails']) {
                        echo "<a href='" . URL . "edit-conf-emails/index/" . $this->data['viewConfEmails'][0]['id'] . "' class='btn-warning'>Editar</a> ";
                    }
                    if ($this->data['button']['edit_conf_emails_password']) {
                        echo "<a href='" . URL . "edit-conf-emails-password/index/" . $this->data['viewConfEmails'][0]['id'] . "' class='btn-warning'>Editar Senha</a> ";
                    }
                    if ($this->data['button']['delete_conf_emails']) {
                        echo "<a href='" . URL . "delete-conf-emails/index/" . $this->data['viewConfEmails'][0]['id'] . "' onclick='return confirm(\"Tem certeza que deseja excluir este registro?\")' class='btn-danger'>Apagar</a> ";
                    }
                }
                ?>
            </div>
